...
data.forEach((quest) => {
    ...
    bilder.src = quest.url

    let figCaption = document.createElement('figcaption')
    figCaption.textContent = quest.question

    figur.appendChild(bilder)
    figur.appendChild(figCaption)

    document.getElementById('content').appendChild(figur)

    let mySection = document.createElement('section')

    quest.choice.forEach((tna) => {
        let myButton = document.createElement('button')
        let myButtonText = document.createTextNode([tna])
...
for (let i = 0; i < allSection.length; i++) {
    ...
    allSection[i].addEventListener('click', (event) => {
        ...
        let resultat = event.target.innerText
